import { getOrCreatePrimaryKey, ensureFreeTrialGranted, consumeCreditForKey, type RequestContext } from "@/lib/api-keys";
 ...
 */
export async function gateScanByCredits(userId: string, scanType: string, ctx?: RequestContext): Promise<ScanGateResult> {
  const apiKeyId = await getOrCreatePrimaryKey(userId);
  await ensureFreeTrialGranted(apiKeyId, ctx?.ip);

  const result = await consumeCreditForKey(apiKeyId, scanType, ctx);
  if (!result.ok) {

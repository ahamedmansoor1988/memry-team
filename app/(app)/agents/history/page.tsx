...
export default function HistoryPage() {
  ...
  if (loading) return (
    <div className="flex-1 flex items-center justify-center text-[13px] text-[#9a9aa5]">Loading history…</div>
  );
  ...
  if (!runs.length) return (
    <div className="flex-1 flex flex-col items-center justify-center gap-3 text-center">
      ...
      <p className="text-[12px] text-[#9a9aa5]">Run a Figma vs Live comparison to see history here.</p>
    </div>
  );
        ...
        {runs.map((run, i) => {
          ...
          return (
            <div key={key} className="rounded-2xl border border-[#f0f0f0] overflow-hidden">
              ...
              >
                {isOpen ? <ChevronDown size={13} className="text-[#9a9aa5] shrink-0" /> : <ChevronRight size={13} className="text-[#9a9aa5] shrink-0" />}
                <div className="flex-1 min-w-0">
                  <p className="text-[12px] font-medium text-[#17171c] truncate">{run.live_url}</p>
                  <p className="text-[11px] text-[#9a9aa5] mt-0.5">{date} at {time}</p>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <a
                    ...
                    className="text-[#9a9aa5] hover:text-[#5b5b66]"
                  >
                  ...
                  <table className="w-full text-[11px]">
                    <thead>
                      <tr className="bg-[#fafafa] border-b border-[#f0f0f0]">
                        <th className="px-4 py-2 text-left font-medium text-[#9a9aa5] w-6">#</th>
                        <th className="px-4 py-2 text-left font-medium text-[#9a9aa5]">Element</th>
                        <th className="px-4 py-2 text-left font-medium text-[#9a9aa5]">Type</th>
                        <th className="px-4 py-2 text-left font-medium text-[#9a9aa5]">Issue</th>
                      </tr>
                    </thead>
                    <tbody>
                      {run.issues.map((issue, j) => {
                        ...
                        return (
                          <tr key={issue.id} style={isHigh ? { backgroundColor: "#fff8f8" } : {}} className="border-b border-[#f7f7f8] last:border-0">
                            <td className="px-4 py-2 text-[#c8c8d0]">{j + 1}</td>
